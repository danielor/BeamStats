...
public class Variance {
  private final static double EPSILON = 1e-7; // For floating point comparisons;
	...
	Combine.AccumulatingCombineFn<N, VarianceAccumulator<N>, Double> of() {
	 ...
	 * @param <N>
	 */
	private static class VarianceFunction<N extends Number> extends Combine.AccumulatingCombineFn<N, VarianceAccumulator<N>, Double> {
    ...
    @Override
		...
		@Override
		public Coder<VarianceAccumulator<N>> getAccumulatorCoder(CoderRegistry registry, Coder<N> inputCoder) {
	 ...
	 * @param <N> Number
	 */
	static class VarianceAccumulator<N extends Number> implements Accumulator<N, VarianceAccumulator<N>, Double> {
		long count = 0;
		double sum = 0.0;
		double sumsq = 0.0;
    ...
    public boolean equals(Object other) {
      ...
      return (count == otherAccum.count) && this.doubleEquals(sum, otherAccum.sum, EPSILON)
          && this.doubleEquals(sumsq, otherAccum.sumsq, EPSILON);
    }
    
    @Override
    public String toString() {
      return String.valueOf(count) + "," + String.valueOf(sum) + "," + String.valueOf(sumsq);
    }
	}
	 ...
	 * @param <N> Number
	 */
	static class VarianceCoder<N extends Number> extends AtomicCoder<VarianceAccumulator<N>> {
	  private static final Coder<Long> LONG_CODER = BigEndianLongCoder.of();
	  private static final Coder<Double> DOUBLE_CODER = DoubleCoder.of();
	  
	  
    @Override
    public void encode(VarianceAccumulator<N> value, OutputStream outStream) throws CoderException, IOException {
    ...
    @Override
    public VarianceAccumulator<N> decode(InputStream inStream) throws CoderException, IOException {
      return new VarianceAccumulator<>(
